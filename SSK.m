...
for ppp=1:1
    ...
    %     json_path_input = 'D:\input\part4.dwg_input.json';
    json_path_input = 'C:\Users\Administrator\AppData\Roaming\WITSOFT\WIT 3D\WIT 3D 2023\output\temp\part4(2).dwg_input.json';
    
    %                json_path_input = 'C:\Users\Administrator\AppData\Roaming\WITSOFT\WIT 3D\WIT 3D 2023\output\temp\ceshi2.dwg_input.json';
    %     json_path_input = 'D:\input\axuediche-2.dwg_input.json';
    ...
    for p = 1:N_part %所排的图的个数
        ...
        I2_x = l_x(I2);
        I2_y = l_y(I2);
        %         beishu = eval([' max(((X_length.' key '/I2_x)/mm_pix),(Y_length.' key '/I2_y)/mm_pix);']);
        %         I2_ttt = imresize(I2,beishu);
        %         beishu_2 = eval(['max(((X_length.' key '/l_x(I2_ttt))/mm_pix),(Y_length.' key '/l_y(I2_ttt))/mm_pix);']);
        ...
        %         beishu_max = eval([' max(((X_length.' key '*imrf/I2_x)/mm_pix),(Y_length.' key '*imrf/I2_y)/mm_pix);']);
        %         I2_TTT = imresize(I2,beishu_max);
        %         beishu_max_2 = eval(['max(((X_length.' key '*imrf/l_x(I2_TTT))/mm_pix),(Y_length.' key '*imrf/l_y(I2_TTT))/mm_pix);']);
        
        I2_t_max = imresize(I2_guang,[ceil(eval(['Y_length.' key])*imrf),ceil(eval(['X_length.' key])*imrf)],'bicubic');
        %         I2_t_max=imresize(I2_guang,[round(eval(['Y_length.' key])*imrf),NaN],'bicubic');
        %         I2_t_max=imresize(I2_t_max,beishu_max_2);
        ...
        %         I2_t_max_or_guang = trans2lefttop(I2_t_max_or);
        %         I2_t_max_or_guang = imcrop(I2_t_max_or_guang,[0,0,l_x(I2_t_max_or_guang),l_y(I2_t_max_or_guang)]);
        %         I2_t_max_or_t = paste_img(I_base,I2_t_max_or_guang,1,1);
        %         I2_t_max_or_t = trans2mid(I2_t_max_or_t,0);
        ...
        I_base = zeros(round(l_max * 1.45),round(l_max * 1.45));
        I2_t_max_t_10 = paste_img(I_base,I2_t_max_t_10,1,1);
        
        %         I2_t_max_or_t_10(I2_t_max_or_t_10 < 0.5) = 0;
        %         I2_t_max_or_t_10(I2_t_max_or_t_10 ~= 0) = 1;
        
        I2_t_max_t_10 = trans2mid(I2_t_max_t_10,0);
        I2_t_max_t_10 =expand(I2_t_max_t_10, partgap * imrf_special);
        I2_t_max_t_10 = trans2mid(I2_t_max_t_10,1);
        %         [nl,nr,nt,nb] = margin(I2_t_max_or_t_10);
        %         middle_coordinate_x = numel(I2_t_max_t_10(1,:)) / (2 * imrf * imrf_special);
        %         middle_coordinate_y = numel(I2_t_max_t_10(:,1)) / (2 * imrf * imrf_special);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        %I2_t_max_10 = expand(I2_t_max_10,partgap*imrf*10);
        %I2_t_max = tuomao(I2_t_max);
        %I2_t_max = logical(I2_t_max);
        I2_t_max_10 = imresize(I2_guang,[ceil(eval(['Y_length.' key]) * imrf * imrf_special),ceil(eval(['X_length.' key]) * imrf * imrf_special)],'bicubic');
        %         I2_t_max_10 = imresize(I2_guang,[round(eval(['Y_length.' key])*imrf*10),NaN],'bicubic');
        I2_t_max_10 = trans2lefttop(I2_t_max_10);
        ...
        I2_t_max_10 = expand(I2_t_max_10,imrf * partgap * imrf_special);
        ...
        S.I1=[];S.I2=[];S.I3=[];
        S.SP=[];S.XP=[];S.ZP=[];S.YP=[];S.N=[];S.ii = [];
        dxy_struct = repmat(S, 4, 1);
        ...
        %         parfor mm = 1:4
        %             if mm == 1
        %                 dxy_struct(mm) = find_best_dxy1(heiban,I0,I2_t0,I2_t,I2_t_max_or,I2_t_max_or_10,...
        %                     Parent0,template,Nmax,key,maxIt_va,nPop,nVar,n,lx1,ly1,nC,nMu,maxIt,W0,L0,kb,partgap,N_part,N_times,imrf_special);
        %             elseif mm == 2
        %                 dxy_struct(mm) = find_best_dxy2(heiban,I0,I2_t0,I2_t,I2_t_max_or,I2_t_max_or_10,...
        %                     Parent0,template,Nmax,key,maxIt_va,nPop,nVar,n,lx1,ly1,nC,nMu,maxIt,W0,L0,kb,partgap,N_part,N_times,imrf_special);
        %             elseif mm == 3
        %                 dxy_struct(mm) = find_best_dxy3(heiban,I0,I2_t0,I2_t,I2_t_max_or,I2_t_max_or_10,...
        %                     Parent0,template,Nmax,key,maxIt_va,nPop,nVar,n,lx1,ly1,nC,nMu,maxIt,W0,L0,kb,partgap,N_part,N_times,imrf_special);
        %             elseif mm == 4
        %                 dxy_struct(mm) = find_best_dxy4(heiban,I0,I2_t0,I2_t,I2_t_max_or,I2_t_max_or_10,...
        %                     Parent0,template,Nmax,key,maxIt_va,nPop,nVar,n,lx1,ly1,nC,nMu,maxIt,W0,L0,kb,partgap,N_part,N_times,imrf_special);
        %             end
        %         end
        ...
        %         mm=5;
        %         imrf=2;
        struct = find_best_dxy5(lx_heiban,ly_heiban,I2_t_max,imrf);
        coordinate_5_ADDI = struct.coordinate;
        coordinate_5_ADDI = sortrows(coordinate_5_ADDI,[1 2]);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %         [~, so] = sort([dxy_struct.N], 'descend'); %so为降序后的序号排序
        %         dxy_best = dxy_struct(so(1));
        %
        ...
        %         I2_t_max = trans2mid(I2_t_max,0);
        %         %         tx=1;%后期需要修改
        %         [dx111,dy111,I1_hr] = high_accuracy1(I2_t_max,alg1_t,key,dx1,dy1,sita10,imrf,0);
        %
        %         %寻找第2次排图的高精度位置
        ...
        %         [dx222,dy222,I2_hr] = high_accuracy2(I1_hr_t,alg2_t,key,dx2,dy2,imrf);%这里的dx2要换
        %
        ...
        %         [dx333_ro,dy333_ro,I3_hr] = high_accuracy3(I2_hr_t,dx3,dy3,sita0,imrf);
        ...
        %         I1_hr_or = I1_hr;
        %         %下面将零件摆平，并寻找合适的间距值
        %         [dx1,dy1,dx2,dy2,dx3,dy3,sita1,ZP,YP,~,~,I1_hr,I2_hr,I3_hr] = baiping(I1_hr,I2_hr,I3_hr,I2_hr_3,I2_hr_3_10,I2_t_max,...
        ...
        %             [coordinate1_pre,coordinate2_pre,~] = paitu_pre(ZP,dx1,dx2,dx3,dy1,dy2,sita1,N_dx3,N_dx3_max,N_shu,heiban,lx_single,ly_single);
        %
        ...
        %                 heiban,I_s_m0,I_s_m_r0,I2_t_temp0,I2_t_max_t_10,I1,ly_single,imrf,imrf_special,I2_t_max_10,I1_hr_10,I2_t_max,I1_hr_or);
        %
        ...
        %             coordinate1_pre(:,4) = coordinate1_pre(:,4) + x_ave1;
        %             coordinate1_pre(:,5) = -coordinate1_pre(:,4) + y_ave1;
        %             coordinate2_pre(:,4) = coordinate2_pre(:,4) + x_ave2;
        %             coordinate2_pre(:,5) = -coordinate2_pre(:,4) + y_ave2;
        %
        ...
        %                 [~,~,~,~,~,N] = paitu(flag_N_h,ZP,n_p,dx1,dx2,dx3,dy1,dy2,dy3,sita1,Nmax,key,...
        %                     N_flag,N_dx3,N_dx3_max,N_shu,heiban,bujin,I_mix,I_single,I_mix_tt,0,I_s_m0,I_s_m_r0,I_accm,I_accm_r,lx_single,ly_single,...
        %                     single,n_w,p,N_part,after_try,lx_add);
        ...
        %             if length(coordinate_single_pre)  > length(coordinate_5_ADDI)
        %                 coordinate_single = coordinate_single_pre(:,1:3);
        %             else
        %                 coordinate_single = coordinate_5_ADDI;
        %             end
        %             coordinate_single = coordinate_single_pre(:,1:3);
        %             save(strcat(outputpath,'\',single_file_name,'.txt'), 'coordinate_single', '-ascii')
        single_file_name = strcat(single_file_name,'A');
        save(strcat(outputpath,'\',single_file_name,'.txt'), 'coordinate_5_ADDI', '-ascii')
        coordinate_single = [];
        %         end
        
        %         while eval(['Nmax.' key ' > 0'])
        %             [coordinate,I_mix,N] = paitu_mix(Nmax,key,sita1,double(I2_t),I_mix,coordinate_single_pre,0);
        ...
        %             end
        %
        %             %             if (eval(['Nmax.' key ' == 0'])) && (p == N_part) %这是针对最后一个零件混排的时候多余的零件,这个时候，即使没有排满一个板子，但是需要输出png和txt
        %             %                 multiple_file_name = strcat('multiple_',nameahead,key,'R_',num_forname);
        %             %                 data_list_o.result.multiple{N_multiple,1}.pngfilepath = strcat(outputpath,'\',multiple_file_name,'.png');
        %             %                 eval(['data_list_o.result.multiple{N_multiple,1}.' key '.txtfilepath = strcat(outputpath,"\",multiple_file_name,".txt");'])
        %             %                 data_list_o.result.multiple{N_multiple,1}.specifysheet = sheetsize;
        ...
        %             end
        %
        %             %             if eval(['Nmax.' key ' == 0'])  && flag_N_h == 1%如果板子铺到最后了，零件也恰好排完，这个时候，有可能还是可以继续排下一个零件的
        %             %                 n_w = n_w + 1;%n_w表示混排的时候，同一个板子上的第几种零件
        %             %             end
        %
        %             %             if eval(['Nmax.' key ' == 0']) && flag_N_h == 0 %某种零件铺完了，但是板子还没有达到最大尺寸，因此需要铺另一种零件，其实可以跟上面的合并
        %             %                 n_w = n_w + 1;%n_w表示混排的时候，同一个板子上的第几种零件
        %             %             end
        %             %         end
        %             %         end
    ...
    output_args = 1;
    time_end = toc;
    disp(['total time used: ',num2str(time_end - time_start)]);

function [Parent,Mother] = Up_pop_Mo2(Mother,Parent,Offspring,nPop,I2_t,I2_t_r,newRc,lx,ly,n,template,Alg2)
%UP_POP 此处显示有关此函数的摘要
%   后期的迭代组合
Nw=sum(I2_t(:))*n;
I=newRc;
...
for i=1:numel(newPop)%如果图元完全分开，则计算方差，否则，将方差置0
    ...
    if Nw_imerged==Nw
        ...
        elseif Alg2==3
            newPop(i).z=(-1)*fun_bs2(I,I22)+(-1)*fun_Variance(I)/100000000;
        elseif Alg2==4
            newPop(i).z=(-1)*fun_Variance(I)+(-1)*fun_bs2(I,I22)/100000000;
        end
...
[~, so]=sort([newPop_t.z], 'descend'); %将newPop_t中的元素按照z值进行排序
